...
# %% auto 0
__all__ = ['cluster_mat', 'generate_clustered_symmetric_matrix', 'plot_matrix', 'get_mat']
...
def generate_clustered_symmetric_matrix(size, cluster_size):
    ...
    if isinstance(cluster_size,list):
        ...
        return reduce(
            np.add,
            [generate_clustered_symmetric_matrix(size, cluster_size=i) for i in cluster_size]
            )
    ...
    for i in clusteris:
        ...
        matrix[start:end, start:end] = cluster * 5  # Scale up cluster values for stronger clustering
...
def get_mat(
    ...
    if cluster:
        ...
        if cluster_sizes is None:
            cluster_sizes=list(np.linspace(size*0.15,size*0.6,10).astype(int))+list(np.linspace(size*0.5,size,5).astype(int))
        matrix=generate_clustered_symmetric_matrix(size, cluster_sizes)    
    # plot_matrix(matrix)
    # Create a pandas DataFrame from the matrix
    np.fill_diagonal(matrix, 0)
    df = pd.DataFrame(matrix, columns=[f"n{i}" for i in range(size)], index=[f"n{i}" for i in range(size)])
    return df
