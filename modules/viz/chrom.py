...
def to_polar(
    ...
    range1 = range1 if range1 is not None else [min(a), max(a)]
    return rescale(
        a,
        range1=range1,
...
def _format_polar_subplot(ax: plt.Axes, xlim: list, **kws_set) -> plt.Axes:
...
def plot_arm(
    ...
    assert data["chromosome"].nunique() == 1, (
        f"multiple chromosomes provided: {data['chromosome'].unique()}"
    )
    ...
    ax.set(
        xlim=_pre_xys(
            [start, end],
            ["tmp"],
            **{k: v for k, v in kws_pre_xys.items() if k != "interval"},
        )[0]
    )
    ...
    if ax.name == "polar":
        ...
        if solid_capstyle != "butt":
            logging.info(
                "For curcular/polar plot, `solid_capstyle` can only be set to 'butt'. Other styles are currently not supported."
            )
            solid_capstyle = "butt"
        if "range1" not in kws_pre_xys:
            kws_pre_xys["range1"] = start, end
        if "range2" not in kws_pre_xys:
            assert pi_span > 0 and pi_span <= 2
            kws_pre_xys["range2"] = _get_pi_range(
                pi_span=pi_span, pi_start=pi_start, pi_end=pi_end
            )
        if "interval" not in kws_pre_xys:
            kws_pre_xys["interval"] = (end - start) / (100 * polar_smoothness_scale)
    else:
        kws_pre_xys["polar"] = False
    if "cytoband type" not in data:
        logging.warning("cytoband type not found in the data")
    else:
        ## cytobands
        data.loc[data["cytoband type"].str.startswith("gpos"), :].apply(
            lambda x: ax.plot(
        ...
        data.loc[data["cytoband type"].str.startswith("gneg"), :].apply(
            lambda x: ax.plot(
                *_pre_xys([x[col_start], x[col_end]], [y, y], **kws_pre_xys),
                lw=lw,
                solid_capstyle="butt",
        ...
        ## centromere
        data.loc[(data["cytoband type"] == "acen"), :].log().apply(
            lambda x: ax.plot(
                *_pre_xys([x[col_start], x[col_end]], [y, y], **kws_pre_xys),
                lw=lw * 0.75,
                solid_capstyle="butt",
        ...
        ## outlines
        start_line, end_line = (
            data.query("`cytoband type`!='acen'")
            .agg({col_start: "min", col_end: "max"})
            .tolist()
        )
        ax.plot(
...
def plot_chrom(
    ...
    **kws_plot_arm,
    ...
    ## subplot
    ax = _get_ax(ax=ax, arc=arc, figsize=figsize)
    if "kws_pre_xys" not in kws_plot_arm:
        kws_plot_arm["kws_pre_xys"] = {}
    if "range2" not in kws_plot_arm["kws_pre_xys"]:
        assert pi_span > 0 and pi_span <= 2
        kws_plot_arm["kws_pre_xys"]["range2"] = _get_pi_range(
            pi_span=pi_span, pi_start=pi_start, pi_end=pi_end
        )
    _ = (
        data.sort_values(col_start)
        .groupby(col_arm, sort=False, observed=False)
        .apply(
            lambda df: plot_arm(
                data=df,
                col_start=col_start,
...
def _rescale_to_chroms(
    ...
    df = df.assign(
        **{
            col_chroms_start: lambda df: df.apply(
                lambda x: x[col_chrom_start] - 1 + x[col_start], axis=1
            ),
        }
    )
    if col_chroms_end is not None:
        df = df.assign(
            **{
                col_chroms_end: lambda df: df.apply(
                    lambda x: x[col_chrom_start] - 1 + x[col_end], axis=1
                ),
            }
        )
    return df
...
def _sort_chroms(
    ...
    data = data.assign(
        **{
            ...
            "chrom name instance": lambda df: df["chromosome"].apply(
    ...
    if chromosomes is None:
        ...
        if len(chromosomes) > 1 and len(chromosomes) != len(chromosomes_all):
            logging.warning(
                f"chromosomes order not provided. inferred one: {chromosomes}"
            )
    else:
    ...
    # sort loci
    data = data.rd.astype_cat(col="chromosome", cats=chromosomes).sort_values(
        ["chromosome"] + ([] if col_start is None else [col_start])
    )
    return data
...
def _concat_chroms(
    ...
    if genome_ends is None:
        genome_ends = (
            data.groupby("chromosome", sort=False, observed=False)[col_end]
            .max()
            .cumsum()
        )
    ...
    df1 = data.assign(
        **{
        ...
        lambda df: _rescale_to_chroms(
            df,
            col_start=col_start,
            col_end=col_end,
            col_chrom_start=col_chrom_start,
            col_chroms_start=col_chroms_start,
            col_chroms_end=col_chroms_end,
        )
    )
    return df1
...
def _set_span_polar(
    ...
    **kws,
    ...
    for xrange in np.column_stack([intervals[:-1], intervals[1:]]):
        getattr(ax, f"ax{span_type}span")(*xrange, **kws)
...
def plot_chroms(
    ...
    **kws_plot_arm,
    ...
    ## chromosomes
    _ = df1.groupby("chromosome", sort=False, observed=False).apply(
        lambda df: plot_chrom(
            data=df,
    ...
    ax.set(
        ...
        else rescale(_xlim, range2=kws_plot_arm["kws_pre_xys"]["range2"])
    )

    df2 = (
        df1.groupby("chromosome", sort=False, observed=False)
        .agg({"genome start": "min", "genome end": "max", "chrom i": "min"})
        .reset_index()
    )
    if ax.name == "polar":
        df_ = df2.apply(
        ...
        if (
            show_vline
            and (
                kws_plot_arm["kws_pre_xys"]["range2"][1]
                - kws_plot_arm["kws_pre_xys"]["range2"][0]
            )
            % (2 * np.pi)
            != 0
        ):
    ...
    if show_labels:
        ...
        df2.apply(
            lambda x: ax.text(
                x=np.mean(
                    [
                        x[f"genome start{' theta' if ax.name == 'polar' else ''}"],
                        x[f"genome end{' theta' if ax.name == 'polar' else ''}"],
                    ]
                ),
                y=label_y,
                s=x["chromosome"]
                if isinstance(x["chromosome"], (int, str))
                else f"{x['chromosome']:.0f}",
                ha="center",
    ...
    if rect is not None:
        ...
        if arc:
            ...
            if offy == 0:
                ...
                if offy is None:
                    offy = off * 0.5
                rect = [-1 * off, (1 - offy), 1 + (2 * off), 1 + (2 * off)]
        else:
