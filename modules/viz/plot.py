...
from chrov.viz.chrom import _get_pi_range, to_polar, _format_polar_subplot
...
def plot_seaborn(
    ...
    **kws_plot,
    ...
    if ax.name == "polar":
        ...
        df0 = df0.assign(
            **{
                colx: lambda df: to_polar(df[colx], range1=range1_chroms, range2=xlim)
    ...
    if kind == "stem":
        ...
        if ax.name != "polar":
            ax.set(
                ylabel=coly,
                ylim=[
                    kws_plot["bottom"] if "bottom" in kws_plot else df0[coly].min(),
                    df0[coly].max(),
                ],
            )
    else:
...
def heatmaps_strips(
    ...
    """
    if fig is None:
        fig, axs = plt.subplots(len(strips_kws), 1, **kws_subplots)
