    ...
    **kws,
    ...
    if ax.name != "polar":
        ...
        if angle <= np.pi / 2:
            ...
            # alignment = "center"
            rotation = (
                rotation + 180 - 2 * abs(rotation - 270)
            )  # 2*abs(rotation-90)# + 2*abs(rotation-90)
...
def annot_labels(
    ...
    if ax_chrom.name != "polar":
        df1 = data.sort_values(colx).assign(
            **{
                col_labelx: lambda df: rescale(
                    df[colx].rank(), range2=ax_chrom.get_xlim()
                )
            }
        )
    else:
        ...
        for name, df in df1.groupby(f"{colx} quadrant", observed=False):
            ...
            range2 = (
                name.left,
                name.right - ((name.right - name.left) / df[colx].nunique()),
            )
            # print(range2)
            l1 += rescale(df[colx].rank(), range2=range2).tolist()
            del range2
    ...
    line1B_yoff = (
        dist * (0.03 if loc == "out" else 0.95)
        if yoff_scales is None
        else yoff_scales[0]
    )
    line2B_yoff = (
        dist * (0.06 if loc == "out" else 0.85)
        if yoff_scales is None
        else yoff_scales[1]
    )
    ## because the polar plots overlap, rectilinear stack
    line3A_yoff = (
        dist * (0.09 if loc == "out" else 0.65)
        if yoff_scales is None
        else yoff_scales[2]
    )
    # if not coly is None:
    ...
    if loc == "out":
        label_yoff = dist * (
            0.12 * (scale_polar if ax_chrom.name == "polar" else 0.8)
            if yoff_scales is None
            else yoff_scales[3]
        )
    else:
    ...
    _ = df1.drop_duplicates(subset=[col_label, col_labelx]).apply(
        lambda x: _set_text(
            ax=ax_chrom,
            x=x[col_labelx],
    ...
    if segments is None:
        ...
        def to_df(segments):
            if isinstance(segments, list):
                df0 = (
                    pd.DataFrame(segments, columns=["start", "end"])
                    .sort_values("start")
                    .reset_index(drop=True)
                )
                df0.index.name = "label"
                df0 = df0.reset_index().assign(
                    label=lambda df: df["label"].apply(lambda x: f"segment{x + 1}")
                )
            elif isinstance(segments, dict):
                df0 = pd.DataFrame(segments, index=["start", "end"]).T
    ...
    if kind == "arrows":
        _ = df0.apply(
            lambda x: ax.annotate(
                text="",
                xy=(x["start"], y if (x.name % 2) == 0 else y - offy),
                xytext=(x["end"], y if (x.name % 2) == 0 else y - offy),
                arrowprops=dict(
                    shrinkA=0,
                    shrinkB=0,
                    **arrow_kws,
