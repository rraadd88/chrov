# AUTOGENERATED! DO NOT EDIT! File to edit: ../../dups/03_isoforms.ipynb.

# %% auto 0
__all__ = ['get_blocks', 'map_feats_to_blocks', 'plot_seq_feats', 'get_plot_data', 'set_plot_data', 'pre_plot_isoforms',
           'pro_plot_isoforms', 'plot_isoforms']

# %% ../../dups/03_isoforms.ipynb 6
## helper functions
...
def pre_plot_isoforms(
    ...
    """Prepares the data for plotting isoforms."""
    
    plot_data = get_plot_data(data=None, feats=feats, force=force)
    data = plot_data['data']
    ...
    if data is None:
        ...
        assert 't.strand' in df3.columns, "t.strand missing after get_ts_data"
        ...
        if feats is not None:
            ...
            if df3 is None:
                logging.error("pre_plot_isoforms: intersect_with_feats: no overlap found")
                return {'data': None, 'feats': None}
            assert 't.strand' in df3.columns, "t.strand missing after intersect_with_feats"
        ...
        if layout == "blocks":
            ...
            if df4 is None:
                logging.error("pre_plot_isoforms: get_blocks: no overlap")
                return {'data': None, 'feats': None}
            assert 't.strand' in df4.columns, "t.strand missing after get_blocks"
            ...
            if df5 is None:
                logging.error("pre_plot_isoforms: map_feats_to_blocks: no overlap")
                return {'data': None, 'feats': None}
            assert 't.strand' in df5.columns, "t.strand missing after map_feats_to_blocks (cds)"
            ...
            if feats is not None:
                ...
                assert 't.strand' in df5.columns, "t.strand missing after map_feats_to_blocks (feats)"
...
def pro_plot_isoforms(
    ...
    **kws_plot_seq_feats,
    ...
    """Plots isoforms from a pre-prepared DataFrame."""
    ...
    xlims = None
    for k, kws in kws_plot.items():
        if k != "plot":
            logging.info(f"plotting {k} ..")
            plot_kws = {**kws_plot["plot"], **kws, **kws_plot_seq_feats}
            if verbose:
                print(
                    kws_plot["plot"],
                    kws,
            ...
            ax = plot_seq_feats(data, ax=ax, **plot_kws)
            if k == "exons":
                xlims = ax.get_xlim()
...
def plot_isoforms(
    ...
    **kws_plot_seq_feats,
    ...
    """
    Main entry point for plotting isoforms.
    This function prepares the data and then calls the plotting function.
    """
    if return_data:
        plot_data = pre_plot_isoforms(
            gene_id=gene_id,
            ensembl_release=ensembl_release,
            species=species,
            layout=layout,
        ...
        return set_plot_data(plot_data['data'], plot_data['feats'])
    ...
    if 't.strand' not in data.columns:
        ts = get_ts(
             gene_id=gene_id,
             ensembl_release=ensembl_release,
             species=species,
        ...
        if ts:
        #     strand = ts[0].strand
        #     data['t.strand'] = strand

        # if ts:
            ts_df = pd.DataFrame(
                [(t.id, t.start, t.end, t.strand) for t in ts],
                columns=['t.id', 't.start', 't.end', 't.strand']
            )
            data = data.drop(columns=['t.start', 't.end', 't.strand'], errors='ignore')
            data = data.merge(ts_df, on='t.id', how='left',validate='m:1')

    ax = pro_plot_isoforms(
        data=data,
        feats=feats,
        layout=layout,
        ax=ax,
        color_exons=color_exons,
        color_cds=color_cds,
