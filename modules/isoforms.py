...
# %% auto 0
__all__ = ['get_blocks', 'map_feats_to_blocks', 'plot_seq_feats', 'get_plot_data', 'set_plot_data', 'plot_isoforms']
...
def plot_seq_feats(
    ...
    **kws_ranges,
    ...
    if order_top is not None:
        order=[s for s in order if s not in order_top]+[s for s in order if s in order_top]
    ...
    df1 = (
          df1
          .assign(y=lambda df: df[col_id].map({s: i for i, s in enumerate(order)}))
        )
    # print(data.columns)
    # print(show_lines)
    if show_lines:
        ...
        kws_plot = {
            ...
            **dict(
                feats=dict(
                    ...
                    hue=None if color_feats not in data else color_feats, ## from col
