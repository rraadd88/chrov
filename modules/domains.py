    ...
    **kws_get_cache,
    ...
    if not Path(outp).exists() or force:
        ...
        if df1["d.start"].min() == 0:
            logging.warning(
                "zero-based indexing found converted to 1-based since these are protein sequences.. "
            )
...
def plot_domains(
    ...
    verbose=False,
    test=False,
    errors="raise",  ## fail-safe to e.g. downloading data
    ax=None,
    kws_get_cache={},
    **kws_ranges,
):
    ...
    if biotype.startswith("p"):
        ...
        if feats is None and data is not None:
            feats = to_gpos(
                data,
                ensembl_release=ensembl_release,
                species=species,
...
def annot_feats(
    ...
    **kws_annot_side_curved,
    ...
    data_back_ = data_back.assign(**{"e.start": lambda df: df["e.start"] - 1})
    ...
    df4 = df3.log.merge(
        ...
        **{
            f"{feat_prefix}{suffix}.center": lambda df: df.loc[
