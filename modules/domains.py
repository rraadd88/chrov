# AUTOGENERATED! DO NOT EDIT! File to edit: ../../dups/04_domains.ipynb.
...
def to_gpos(
    ...
    df4_ = (
        df3
        .groupby("p.id")
        .apply(lambda df: get_protein_cds_coords(
            annots=annots,
            protein_id=df.name,
            )
            )
...
def annot_feats(
    ...
    **kws_annot_side_curved,
    ...
    cols=['t.id','t.start','t.end','e.id','e.start','e.end']+(['eb.id','eb.start','eb.end'] if layout=='blocks' else [])
    # print(cols)
    
    data_back=(
            data
            .query(expr="`data type`=='data'").drop(['data type'],axis=1)
    ...
    seqid_to_y={o.get_text().strip():o.get_position()[1] for o in os if o.get_text().strip()!=''}
        ...
        .assign(
            **{
                f"{feat_prefix}{suffix}.center":lambda df:
                    ...
                    .loc[:,
                     [
                        f"{feat_prefix}{suffix}.start",
                        f"{feat_prefix}{suffix}.end"]
                     ]
                    .mean(axis=1)
                    ),
            }            
