...
def get_data_byeids(
    ...
    return (
        ...
        .loc[
            :,
            data.filter(regex="^b.*").columns.tolist()+data.filter(regex="^eb.*").columns.tolist()+['e.id'],
        ]
        .sort_values('b.id')
        .dropna(
            # subset=['e.id']
        ...
        .drop(
            [
                'e.id',
                "eb.overlap","eb.start","eb.end raw","eb.end","eb.id","eb.length",
            ],
            axis=1,
        )
        .drop_duplicates()
...
def plot_forms(
    ...
    **kws_plot_domains,
    ...
    if return_data_for_mapping:
        assert layout.lower().startswith('b'), "return_data_for_mapping is only developed for blocks"
        return_data=True
...
def get_labels_pos(
    ...
    pd.DataFrame(
        ...
        {t.get_text().strip():t.get_position() for t in ax_gv.get_children() if isinstance(t,plt.Text) if t.get_text()!='' if '-' not in t.get_text()}
    )
    .T
    .rename(
        columns={
